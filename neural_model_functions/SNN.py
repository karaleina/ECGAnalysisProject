...
class NeuralNetwork:
    ...
    def fit(self, X, y, learning_rate=0.1, epochs=1000000):
        ...
        for k in range(epochs):
            ...
            for l in range(len(self.__weights)):
                dot_value = np.dot(a[l], self.__weights[l])
            ...
            for l in range(len(a) - 2, 0, -1):
                deltas.append(deltas[-1].dot(self.__weights[l].T) * self.activation_prime(a[l]))
            ...
            for i in range(len(self.__weights)):
                ...
                self.__weights[i] += learning_rate * layer.T.dot(delta)

    def predict(self, x):
        a = np.concatenate((np.ones(1).T, np.array(x)), axis=0)
        for l in range(0, len(self.__weights)):
            a = self.__activation(np.dot(a, self.__weights[l]))
        return a
