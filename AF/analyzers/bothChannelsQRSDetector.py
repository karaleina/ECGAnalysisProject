...
class BothChannelsQRSDetector(object):
    ...
    def compare(self, signals, sampling_ratio=250, tol_compare_time=0.1, start_sample=0, stop_sample=1000000000000,
        ...
        self._tol_compare_time = tol_compare_time
        self._record_file = record_file
        self._start_sample = start_sample
        self._stop_sample = stop_sample

        hamilton = r_wave_detection.Hamilton()#QRSPanThompkinsDetector()
        dc = detection_combiner.DetectionCombiner()
        old_channel_r_waves = []

        for signal_index in range(self._number_of_signals):
            r_waves = np.array(hamilton.detect_r_waves(self._signals[:, signal_index], sampling_rate=sampling_ratio)).ravel()
            new_channel_r_waves = dc.verify(r_waves, sampling_ratio=self._sampling_ratio,
                                         tol_compare_time=self._tol_compare_time)
            self._combined_rr = dc.combine(sampling_ratio=sampling_ratio, channel1=old_channel_r_waves,
                                           channel2=new_channel_r_waves)
